...
public class EmailSignUpActivity extends BaseActivity {
    ...
    private void AddUserData(FirebaseUser user,String name) {
        ...
        userProfile.setLoginType(UserProfile.EMAIL);
    ...
    public void SendEmailVerifi(final FirebaseUser user) {
    ...
    private void dialogResetPassword() {
        new AlertDialog.Builder(this,R.style.AppTheme_Dark_Dialog)
                .setTitle("ไม่สามารถใช้งานที่อยู่อีเมล์นี้ได้")
                .setMessage("เนื่องจากที่อยู่อีเมลถูกใช้โดยบัญชีอื่นแล้ว กรุณาลองใหม่อีกครั้ง")
                .setPositiveButton("ตกลง", null)
                .setNegativeButton("รีเซ็ตรหัสผ่าน", new DialogInterface.OnClickListener() {
                    @Override
                    public void onClick(DialogInterface dialogInterface, int i) {
                        Intent intent = new Intent(EmailSignUpActivity.this, RecoveryActivity.class);
